    ...
    return (
        <div>
            <div className='w-full h-auto'>
                <div className='hidden md:flex grid grid-cols-1 gap-5 place-items-center'>
                ...
                <div className='md:hidden'>
                    {informations.map((info, index) => (
                        <div key={index} className='w-full h-auto py-2 flex flex-col justify-between border-b rounded-xs cursor-pointer'>
                            <div className='flex' onClick={() => navigate(`/informations/${info._id}`)}>
                                <div className='w-2/5 h-1/5'>
                                    <img src={info.image} className='w-full h-full object-cover object-center' />
                                </div>
                                <div className='flex flex-col justify-center px-4'>
                                    <p className='text-sm lg:text-sm h-10 font-semibold'>{info.title}</p>
                                    <p className='text-[10px] lg:text-sm text-gray-500 py-4'>{new Date(info.createdAt).toLocaleDateString()}</p>
                                </div>
                            </div>
                        </div>
