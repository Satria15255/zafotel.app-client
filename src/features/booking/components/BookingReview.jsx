...
const BookingReview = () => {
    ...
    return (
        <section className="w-full flex flex-col items-center">
            ...
            <main className="h-auto w-full p-4 md:p-10 xl:max-w-7xl">
                <div className="grid grid-cols-1 md:grid-cols-2 border rounded-xl">
                    <div className="w-full h-60 lg:h-80 p-2">
                        <img
                            src={room.image[0]}
                            alt={room.name}
                            className="w-full h-full rounded-lg md:rounded-l-xl object-cover"
                        />
                    </div>
                            ...
                            {room.details.amenities
                                ...
                                .map((amenities) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {FacilityIcon[key]}
                                            <p className="text-xs md:text-sm lg:text-lg items-center py-3">
                                                {amenities}
                                            </p>
                                        </div>
                                    );
                ...
                <div className="space-y-3 mt-2 border rounded-lg px-6 py-2">
                    <p className="text-xl font-serif mt-7 font-semibold">
                        Booking Details
                    </p>
                    <p className="flex justify-between text-sm md:text-lg">
                        Your Name :<span>{state.userName}</span>
                    </p>
                    <p className="flex justify-between text-sm md:text-lg">
                        Phone Number :<span>{state.phoneNumber}</span>
                    </p>
                    <p className="flex justify-between text-sm md:text-lg">
                        Check-in Date :<span>{state.checkInDate}</span>
                    </p>
                    <p className="flex justify-between text-sm md:text-lg">
                        Check-Out Date :<span>{state.checkOutDate}</span>
                    </p>
                    <p className="flex justify-between text-sm md:text-lg">
                        Units Booked :
                        <span>{Number(state.unitsBooked)} Units</span>
                    </p>
                    <p className="flex justify-between text-sm md:text-lg">
                        Total Payment :
